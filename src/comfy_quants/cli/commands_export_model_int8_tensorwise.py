"""export-model-int8-tensorwise command (stock-ComfyUI int8, + optional ConvRot)."""
...
from comfy_quants.formats.int8_tensorwise import INT8_TENSORWISE_FORMAT_NAME
...
_DEFAULT_CHECKPOINT_NAME = f"diffusion_pytorch_model.{INT8_TENSORWISE_FORMAT_NAME}.safetensors"


def register(subparsers):
    parser = subparsers.add_parser(
        "export-model-int8-tensorwise",
        help="Export a full stock-ComfyUI int8 (+ConvRot) inference checkpoint (ComfyUI>=0.27, SM>=7.5)",
    )
...
def _build_tensor_index(cfg, source: Path) -> dict:
    ...
    # Stock int8_tensorwise weights are per-output-channel regardless of the config
    # scale section (ConvRot requires per-channel in comfy-kitchen).
...
def run(args) -> int:
    ...
    if cfg.quant.target_dtype != INT8_TENSORWISE_FORMAT_NAME:
        raise ConfigurationError(
            f"export-model-int8-tensorwise requires quant.target_dtype '{INT8_TENSORWISE_FORMAT_NAME}', got {cfg.quant.target_dtype}"
        )
