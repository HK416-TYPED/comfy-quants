"""Safetensors stock-ComfyUI INT8 (int8_tensorwise, + optional ConvRot) exporter.

OFFLINE producer of checkpoints loadable by stock ComfyUI's built-in
``QUANT_ALGOS["int8_tensorwise"]`` path (ComfyUI >= v0.27.0). Mirrors the INT8-Fast
writer (:mod:`comfy_quants.backends.int8_w8a8_model_export`) in structure, but the
marker and the quantization math follow the STOCK contract:

- ``comfy_quant`` marker carries ``{"format": "int8_tensorwise"}`` plus ``convrot`` /
  ``convrot_groupsize`` only when the layer was rotated — byte-matching stock
  ComfyUI's own save path (``ops.py:_quantized_weight_state_dict``).
- Quantization is bit-faithful to comfy-kitchen's eager
  ``quantize_int8_convrot_weight`` / ``quantize_int8_rowwise`` **as of >= 0.2.15**
  (the layout itself shipped in 0.2.11, but the dtype-preserving division recipe —
  ``_int8_scale_for_math`` — landed in 0.2.15; 0.2.12..0.2.14 divide in fp32, which
  differs by at most ±1 int8 code on bf16/fp16 weights): the Hadamard is built and
  applied at the SOURCE WEIGHT dtype (not fp32), the fp32 per-row scale is
  ``amax.float()/127`` clamped to >= 1e-30, and the weight is DIVIDED by the scale
  cast back to the weight dtype (zeros replaced by tiny), then round-to-nearest-even
  and clamped. Once ComfyUI's comfy-kitchen pin is >= 0.2.15 this matches what
  ComfyUI itself produces when re-quantizing (e.g. on LoRA offload); it deviates
  from the INT8-Fast writer's cast-to-fp32 + reciprocal-multiply recipe.

Emits NO ``input_scale`` (activations are quantized dynamically inside the
...
from comfy_quants.formats.int8_tensorwise import (
...
@dataclass
class Int8TensorwiseCheckpointExportReport:
    """Summary of a full stock-ComfyUI int8_tensorwise checkpoint export."""
...
def _resolve_int8_tensorwise_target_dtype(tensor_index: dict[str, Any], target_dtype: str | None = None) -> str:
    index_dtype = tensor_index.get("format", {}).get("name")
    resolved = target_dtype or index_dtype
    if resolved != INT8_TENSORWISE_FORMAT_NAME:
        raise PayloadWriteError(f"tensor index is not an {INT8_TENSORWISE_FORMAT_NAME} format: {resolved}")
    if index_dtype != INT8_TENSORWISE_FORMAT_NAME:
        raise PayloadWriteError(f"tensor index format {index_dtype} does not match requested target dtype {resolved}")
    return INT8_TENSORWISE_FORMAT_NAME
...
def _check_selected_rows(tensor_index: dict[str, Any], *, target_dtype: str) -> dict[str, dict[str, Any]]:
    ...
    for row in list(tensor_index.get("tensors") or []):
        if row.get("quant_dtype") != target_dtype:
            raise PayloadWriteError(f"unsupported quant dtype for int8_tensorwise export: {row.get('quant_dtype')}")
        if row.get("storage_dtype") != "int8":
            raise PayloadWriteError(f"unsupported storage dtype for int8_tensorwise export: {row.get('storage_dtype')}")
...
def _quantize_int8_tensorwise_per_row(tensor, *, convrot: bool, group_size: int) -> tuple[Any, Any, bool]:
    ...
    comfy-kitchen eager ``quantize_int8_convrot_weight``/``quantize_int8_rowwise``:
    Hadamard built and applied at the weight dtype, ``scale = amax.float()/127``
    clamped to >= 1e-30, quantization by DIVISION with the scale cast back to the
    weight dtype (zeros -> dtype tiny), round-half-to-even, clamp [-128, 127].
    ConvRot gated on ``in % group_size == 0`` (comfy-kitchen raises there; this
    writer skips rotation for such layers and omits the marker's convrot keys).
    """
    torch = _require_torch()
    w = tensor.detach()
    if w.dim() != 2:
        raise PayloadWriteError("int8_tensorwise export requires a rank-2 weight tensor")
    rotated = False
    ...
    quant = (w / scale_math).round_().clamp_(-128.0, 127.0).to(torch.int8)
    return quant.contiguous(), scale.to(torch.float32).contiguous(), rotated


def write_int8_tensorwise_inference_checkpoint_from_safetensors(
    *,
...
) -> Int8TensorwiseCheckpointExportReport:
    """Write a full checkpoint with selected weights stored as stock int8 tensors."""
    safe_open, save_file = _require_safetensors()
    torch = _require_torch()
    resolved_target_dtype = _resolve_int8_tensorwise_target_dtype(tensor_index, target_dtype)
    ...
    output_metadata.update(
        {
            ...
            # Family-agnostic on purpose — deliberate break from the older writers'
            # hardcoded qwen_image_ prefix (a documented wart, not a contract).
    ...
    return Int8TensorwiseCheckpointExportReport(
